 ...
 *
 * 重构前这条路上有两个各自为政的调用者，都直接调 [RadioSource.fetchPlaybill]：
 * 节目单面板（`loadPlaybill`）和直播进度条的当前节目窗口（`currentProgramWindow`）。
 * 用户在正播的台上打开节目单，两者要的是**同一天、同一个台**的同一份数据，却是两次网络往返；
 * 遥控快切日期再切回来还会把刚拿到的那份重新拉一遍。弱性能 TV 上这些都是白花的
 * JSON 解析与 GC。
 *
 * 三条语义写在一处，才好一次性证明它们成立：
...
class ProgramRepository(
     ...
     * 副标题和进度条会一直停在上一节目 —— 正是重构前靠「窗口未知也重试」躲开的那个坑。
     * 故未覆盖此刻时无视 TTL 重新取。重试频率由调用方节流
     * （`RadioViewModel.LIVE_RESOLVE_MIN_INTERVAL_MS`），此处不再叠一层。
     *
