...
class GatewayByIdsTest {
    ...
    fun `by-ids 带上 catalog 范围，刷新不依赖地区码填得对不对`() = runTest {
        ...
        val src = GatewaySource(api)

        src.fetchChannelsByIds(RadioSourceType.YUNTING, 110000, listOf("gd1"))
    ...
    fun `id 列表按逗号拼接下发`() = runTest {
        ...
        val out = src.fetchChannelsByIds(RadioSourceType.YUNTING, 407, listOf("a", "b", "c"))
    ...
    fun `空 id 列表直接短路，不发请求`() = runTest {
        ...
        assertEquals(emptyList<Channel>(), src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, emptyList()))
    ...
    fun `端点未部署时返回空，且不再重复请求`() = runTest {
        ...
        assertEquals(emptyList<Channel>(), src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, listOf("a")))
        assertEquals(emptyList<Channel>(), src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, listOf("b")))
        assertEquals(emptyList<Channel>(), src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, listOf("c")))
    ...
    fun `能力开关跨来源共享，另一个来源不必再撞一次 404`() = runTest {
        ...
        src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, listOf("a"))
        src.fetchChannelsByIds(RadioSourceType.QINGTING, 407, listOf("b"))
        src.fetchChannelsByIds(RadioSourceType.TUNEIN, 100436, listOf("c"))
    ...
    fun `其他 HTTP 错误照常上抛，不误判成能力缺失`() = runTest {
        ...
        val thrown = runCatching { src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, listOf("a")) }.exceptionOrNull()
        assertTrue("503 是偶发故障，不该关掉能力开关，实际 $thrown", thrown is HttpException)

        // 开关没被关掉，下一次仍会正常尝试。
        runCatching { src.fetchChannelsByIds(RadioSourceType.YUNTING, 0, listOf("b")) }
