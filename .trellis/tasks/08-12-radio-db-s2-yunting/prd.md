...
- [x] 装载跑通真上游:943 台入库(并集正确,两边各自独有的 5 台 + 1 台都在)
- [x] `provinceCode=0` 的 19 个全国台在库里且能被浏览到
- [x] 分类联结表覆盖那 125 台多分类电台
- [x] 库里**没有**任何云听播放地址
- [x] `/v1/stream?source=yunting&contentId=X` 返回可播地址,按省缓存 30 分钟生效
      (**不是** `scopeCache`:切库后那一层装的是库里查出来的行,没有 `playUrlLow`)
- [x] APP 侧无改动;起播地址服务端侧已验(解析出的地址真拉 200)—— **真机验收待做**
- [x] subtitle 来自 `listByDate` 推算(315/943 台有节目单,其余上游本就没排,
      切库前显示的也是「暂无节目单」;APP 侧 `ifBlank` 补,用户无感)
- [x] 读取切库前后逐字节等价性验证通过
- [x] 定时任务在位,失败可被发现
